...
import { Program } from '../lib/airtable';
...
export function ProgramForm({ program, onSuccess, onCancel }: ProgramFormProps) {
  const [formData, setFormData] = useState({
    ...
    state: program?.state || '',
    mileageFee: program?.mileageFee || 0,
    tripFee: program?.tripFee || 0,
    childFee: program?.childFee || 0,
    waitFee: program?.waitFee || 0,
    wheelchairFee: program?.wheelchairFee || 0,
    notes: program?.notes || '',
    status: program?.status || 'Active',
    contractUrl: program?.contractUrl || ''
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.contractNumber || !formData.entity || !formData.city || !formData.county || !formData.state) {
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-8">
        ...
        <div className="flex justify-end space-x-3">
          ...
          <Button
            ...
            disabled={loading}
          >
            {loading ? 'Saving...' : program?.id ? 'Update Program' : 'Add Program'}
          </Button>
        </div>
      </form>
    </div>
